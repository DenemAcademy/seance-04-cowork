...
CHAPTERS = [
    ...
    (
        ...
        [
            ...
            section("La réponse finale", "12:57", 15, "Après l’exécution, Cowork annonce ce qu’il a fait. Je vérifie ensuite le résultat dans Google Calendar, parce qu’une IA ne doit jamais être validée sans contrôle.", "Ne vous arrêtez pas au message de succès. Ouvrez l’outil cible et vérifiez que le résultat existe vraiment.", "Le résultat est visible hors de Cowork.", "Pour Calendar, ouvrez la semaine concernée et regardez les événements."),
    ...
    (
        ...
        [
            ...
            section("Limiter les connecteurs utilisés", "11:46", 12, "L’interface affiche plusieurs connecteurs, mais je précise que l’exercice n’a besoin que de Google Calendar.", "Quand une tâche ne demande qu’un outil, dites-le. Cela évite que Cowork cherche dans Drive, Notion ou d’autres outils sans raison.", "Le connecteur utile est bien celui utilisé.", "Phrase pratique : pour cette tâche, utilise seulement Google Calendar."),
    ...
    (
        ...
        "Après Calendar, je vous montre que Cowork peut aussi produire des pages, travailler avec Drive et créer des documents dans le dossier.",
        "Documents",
        "orange",
        [
            section("La suite avec Notion", "13:39", 15, "Après les événements Calendar, je demande de créer une page Notion avec le même contenu. Je rappelle qu’il faut connecter Notion comme Calendar.", "Connectez Notion avant de demander une création. Le système reste le même : connecteur, autorisation, tâche, vérification.", "Notion est connecté avant la demande.", "Sans connecteur Notion, Cowork peut proposer une autre solution mais ne pourra pas créer directement dedans."),
            section("Créer un récap visuel", "14:47", 15, "Le résultat Notion sert à avoir une vue propre de la semaine. J’insiste sur le côté visuel et organisé parce que votre futur système IA doit rester lisible.", "Demandez un format clair : titre, semaine, tableau, checklist, prochaines actions.", "Le récap peut être relu rapidement.", "Une page de suivi doit aider à agir, pas seulement faire joli."),
            section("Centraliser les sorties", "14:20", 15, "Je reviens sur le besoin de centraliser : planifier, créer, résumer et garder les informations dans une même app ou un même QG.", "Quand Cowork produit quelque chose, décidez où cette sortie doit vivre : Calendar, Notion, Drive ou dossier local.", "Chaque sortie a une destination claire.", "Un résumé client sans destination finit souvent oublié."),
            section("Le connecteur Drive", "16:13", 9, "Je vous montre ensuite l’idée d’utiliser Drive pour retrouver ou créer des informations. Je tente de gérer le connecteur pour que vous voyiez aussi qu’un problème peut arriver.", "Si Drive ne répond pas comme prévu, ne forcez pas. Vérifiez le compte connecté, les droits et la disponibilité des documents.", "Le problème est identifié avant de changer de stratégie.", "Si le Drive est vide ou mal connecté, Cowork ne peut pas inventer les fichiers."),
            section("Le bug comme apprentissage", "18:26", 9, "Je montre une difficulté en live parce que c’est utile : les connecteurs ne marchent pas toujours parfaitement du premier coup.", "Quand une erreur arrive, gardez une méthode : lire le message, vérifier le compte, vérifier les droits, tester une demande plus simple.", "La correction suit une logique, pas des clics au hasard.", "Test simple : liste les fichiers récents du Drive connecté."),
            ...
            section("La création de document", "19:18", 15, "Je vous montre que Cowork peut créer un document, installer un package et produire un fichier dans le dossier. Cela rejoint l’idée de QG local.", "Pour un document local, demandez le nom du fichier, le format et l’endroit où l’enregistrer.", "Le fichier existe dans le dossier QG.", "Exemple : crée un document de récap de la semaine de prospection dans le QG."),
    ...
    (
        ...
        [
            ...
            section("La routine vérification", "12:57", 15, "Chaque action externe doit être vérifiée. Je le fais avec Google Calendar après la création des événements pour que vous gardiez ce réflexe.", "Vérifiez dans l’outil réel, puis corrigez si besoin. Ne laissez pas Cowork être le seul juge de son résultat.", "Le résultat existe et correspond à la demande.", "Une réponse de succès ne remplace pas un contrôle dans Calendar."),
...
def section_html(section_data: dict[str, str | int], number: int) -> str:
    ...
    recap = f"""
      <div class="mt-6 border-2 border-slate-950 bg-slate-950 p-5 text-white shadow-neo">
        ...
        <p class="mt-3 leading-8">À ce moment de la séance, je veux que vous reteniez une action simple, vérifiable et réutilisable dans votre QG.</p>
      </div>
    """
      ...
      <div class="mx-auto grid min-h-[82vh] max-w-7xl gap-10 px-4 py-16 sm:px-6 lg:grid-cols-[1fr_.9fr] lg:items-center lg:px-8">
        <div>
          ...
          <p class="mt-6 max-w-2xl text-xl leading-9 text-slate-700">
            Dans cette séance, je vous montre comment utiliser Claude Desktop et Cowork pour centraliser votre travail : dossier QG, connecteurs, Google Calendar, Notion, Drive, compétences et plugins.
          </p>
          <div class="mt-8 border-2 border-slate-950 bg-violet-50 p-5 shadow-neo">
            ...
            <p class="mt-3 leading-8 text-slate-800">Vous partez d’un cas concret : organiser un business et des projets clients. Vous créez un QG, vous connectez les outils, vous planifiez une semaine de prospection, puis vous vérifiez le résultat comme un vrai futur expert IA.</p>
          </div>
